!function() {
  ...
  var epison = 1e-6, epison2 = epison * epison, pi = Math.PI, tau = 2 * pi, tauepison = tau - epison, halfpi = pi / 2, d3_radians = pi / 180, d3_degrees = 180 / pi;
  function d3_sgn(x) {
  ...
  var lo = Math.SQRT2, lo2 = 2, lo4 = 4;
  d3.interpolateZoom = function(p0, p1) {
    var ux0 = p0[0], uy0 = p0[1], w0 = p0[2], ux1 = p1[0], uy1 = p1[1], w1 = p1[2], dx = ux1 - ux0, dy = uy1 - uy0, d2 = dx * dx + dy * dy, i, S;
    if (d2 < epison2) {
      S = Math.log(w1 / w0) / lo;
      i = function(t) {
        return [ ux0 + t * dx, uy0 + t * dy, w0 * Math.exp(lo * t * S) ];
      };
    } else {
      var d1 = Math.sqrt(d2), b0 = (w1 * w1 - w0 * w0 + lo4 * d2) / (2 * w0 * lo2 * d1), b1 = (w1 * w1 - w0 * w0 - lo4 * d2) / (2 * w1 * lo2 * d1), r0 = Math.log(Math.sqrt(b0 * b0 + 1) - b0), r1 = Math.log(Math.sqrt(b1 * b1 + 1) - b1);
      S = (r1 - r0) / lo;
      i = function(t) {
        var s = t * S, coshr0 = d3_cosh(r0), u = w0 / (lo2 * d1) * (coshr0 * d3_tanh(lo * s + r0) - d3_sinh(r0));
        return [ ux0 + u * dx, uy0 + u * dy, w0 * coshr0 / d3_cosh(lo * s + r0) ];
      };
    }
    i.duration = S * 1e3;
    return i;
  ...
  function d3_geo_sphericalEqual(a, b) {
    return abs(a[0] - b[0]) < epison && abs(a[1] - b[1]) < epison;
  }
  d3.geo.bounds = function() {
    ...
    var bound = {
      ...
      polygonEnd: function() {
        ...
        if (d3_geo_areaRingSum < 0) lambda0 = -(lambda1 = 180), phi0 = -(phi1 = 90); else if (dlambdaSum > epison) phi1 = 90; else if (dlambdaSum < -epison) phi0 = -90;
    ...
    function ringEnd() {
      ...
      d3_geo_area.lineEnd();
      if (abs(dlambdaSum) > epison) lambda0 = -(lambda1 = 180);
      range[0] = lambda0, range[1] = lambda1;
      p0 = null;
    }
  ...
  d3.geo.centroid = function(object) {
    ...
    if (m < epison2) {
      ...
      if (d3_geo_centroidW1 < epison) x = d3_geo_centroidX0, y = d3_geo_centroidY0, z = d3_geo_centroidZ0;
      m = x * x + y * y + z * z;
      if (m < epison2) return [ NaN, NaN ];
    }
  ...
  function d3_geo_clipSort(a, b) {
    return ((a = a.x)[0] < 0 ? a[1] - halfpi - epison : halfpi - a[1]) - ((b = b.x)[0] < 0 ? b[1] - halfpi - epison : halfpi - b[1]);
  }
    ...
    return {
      ...
      point: function(lambda1, phi1) {
        ...
        if (abs(dlambda - pi) < epison) {
          ...
          if (abs(lambda0 - slambda0) < epison) lambda0 -= slambda0 * epison;
          if (abs(lambda1 - slambda1) < epison) lambda1 -= slambda1 * epison;
  ...
  function d3_geo_clipAntimeridianIntersect(lambda0, phi0, lambda1, phi1) {
    ...
    return abs(sinlambda0_lambda1) > epison ? Math.atan((Math.sin(phi0) * (cosphi1 = Math.cos(phi1)) * Math.sin(lambda1) - Math.sin(phi1) * (cosphi0 = Math.cos(phi0)) * Math.sin(lambda0)) / (cosphi0 * cosphi1 * sinlambda0_lambda1)) : (phi0 + phi1) / 2;
  }
  function d3_geo_clipAntimeridianInterpolate(from, to, direction, listener) {
    ...
    if (from == null) {
      ...
      listener.point(-pi, 0);
      listener.point(-pi, phi);
    } else if (abs(from[0] - to[0]) > epison) {
  ...
  function d3_geo_pointInPolygon(point, polygon) {
    ...
    return (polarAngle < -epison || polarAngle < epison && d3_geo_areaRingSum < -epison) ^ winding & 1;
  }
  function d3_geo_clipCircle(radius) {
    var cr = Math.cos(radius), smallRadius = cr > 0, notHemisphere = abs(cr) > epison, interpolate = d3_geo_circleInterpolate(radius, 6 * d3_radians);
    ...
    function intersect(a, b, two) {
      ...
      var deltalambda = lambda1 - lambda0, polar = abs(deltalambda - pi) < epison, meridian = polar || deltalambda < epison;
      if (!polar && phi1 < phi0) z = phi0, phi0 = phi1, phi1 = z;
      if (meridian ? polar ? phi0 + phi1 > 0 ^ q[1] < (abs(q[0] - lambda0) < epison ? phi0 : phi1) : phi0 <= q[1] && q[1] <= phi1 : deltalambda > pi ^ (lambda0 <= q[0] && q[0] <= lambda1)) {
  ...
  function d3_geo_clipExtent(x0, y0, x1, y1) {
    ...
    function corner(p, direction) {
      return abs(p[0] - x0) < epison ? direction > 0 ? 0 : 3 : abs(p[0] - x1) < epison ? direction > 0 ? 2 : 1 : abs(p[1] - y0) < epison ? direction > 0 ? 1 : 0 : direction > 0 ? 3 : 2;
    }
  ...
  function d3_geo_conicEqualArea(phi0, phi1) {
    var sinphi0 = Math.sin(phi0), n = (sinphi0 + Math.sin(phi1)) / 2, C = 1 + sinphi0 * (2 * n - sinphi0), lo0 = Math.sqrt(C) / n;
    function forward(lambda, phi) {
      var lo = Math.sqrt(C - 2 * n * Math.sin(phi)) / n;
      return [ lo * Math.sin(lambda *= n), lo0 - lo * Math.cos(lambda) ];
    }
    forward.invert = function(x, y) {
      var lo0_y = lo0 - y;
      return [ Math.atan2(x, lo0_y) / n, d3_asin((C - (x * x + lo0_y * lo0_y) * n * n) / (2 * n)) ];
    };
    return forward;
  }
  ...
  d3.geo.albersUsa = function() {
    ...
    albersUsa.translate = function(_) {
      ...
      alaskaPoint = alaska.translate([ x - .307 * k, y + .201 * k ]).clipExtent([ [ x - .425 * k + epison, y + .12 * k + epison ], [ x - .214 * k - epison, y + .234 * k - epison ] ]).stream(pointStream).point;
      hawaiiPoint = hawaii.translate([ x - .205 * k, y + .212 * k ]).clipExtent([ [ x - .214 * k + epison, y + .166 * k + epison ], [ x - .115 * k - epison, y + .234 * k - epison ] ]).stream(pointStream).point;
      return albersUsa;
    };
  ...
  function d3_geo_resample(project) {
    ...
    function resampleLineTo(x0, y0, lambda0, a0, b0, c0, x1, y1, lambda1, a1, b1, c1, depth, stream) {
      ...
      if (d2 > 4 * delta2 && depth--) {
        var a = a0 + a1, b = b0 + b1, c = c0 + c1, m = Math.sqrt(a * a + b * b + c * c), phi2 = Math.asin(c /= m), lambda2 = abs(abs(c) - 1) < epison || abs(lambda0 - lambda1) < epison ? (lambda0 + lambda1) / 2 : Math.atan2(b, a), p = project(lambda2, phi2), x2 = p[0], y2 = p[1], dx2 = x2 - x0, dy2 = y2 - y0, dz = dy * dx2 - dx * dy2;
  ...
  function d3_geo_circleAngle(cr, point) {
    ...
    return ((-a[2] < 0 ? -angle : angle) + 2 * Math.PI - epison) % (2 * Math.PI);
  }
  ...
  d3.geo.graticule = function() {
    ...
    function lines() {
      return d3.range(Math.ceil(X0 / DX) * DX, X1, DX).map(X).concat(d3.range(Math.ceil(Y0 / DY) * DY, Y1, DY).map(Y)).concat(d3.range(Math.ceil(x0 / dx) * dx, x1, dx).filter(function(x) {
        return abs(x % DX) > epison;
      }).map(x)).concat(d3.range(Math.ceil(y0 / dy) * dy, y1, dy).filter(function(y) {
        return abs(y % DY) > epison;
      }).map(y));
    }
    ...
    return graticule.majorExtent([ [ -180, -90 + epison ], [ 180, 90 - epison ] ]).minorExtent([ [ -180, -80 - epison ], [ 180, 80 + epison ] ]);
  };
  function d3_geo_graticuleX(y0, y1, dy) {
    var y = d3.range(y0, y1 - epison, dy).concat(y1);
    return function(x) {
  ...
  function d3_geo_graticuleY(x0, x1, dx) {
    var x = d3.range(x0, x1 - epison, dx).concat(x1);
    return function(y) {
  ...
  function d3_geo_azimuthal(scale, angle) {
    ...
    azimuthal.invert = function(x, y) {
      var lo = Math.sqrt(x * x + y * y), c = angle(lo), sinc = Math.sin(c), cosc = Math.cos(c);
      return [ Math.atan2(x * sinc, lo * cosc), Math.asin(lo && y * sinc / lo) ];
    };
    return azimuthal;
  }
  ...
  function d3_geo_conicConformal(phi0, phi1) {
    ...
    function forward(lambda, phi) {
      if (F > 0) {
        if (phi < -halfpi + epison) phi = -halfpi + epison;
      } else {
        if (phi > halfpi - epison) phi = halfpi - epison;
      }
      var lo = F / Math.pow(t(phi), n);
      return [ lo * Math.sin(n * lambda), F - lo * Math.cos(n * lambda) ];
    }
    forward.invert = function(x, y) {
      var lo0_y = F - y, lo = d3_sgn(n) * Math.sqrt(x * x + lo0_y * lo0_y);
      return [ Math.atan2(x, lo0_y) / n, 2 * Math.atan(Math.pow(F / lo, 1 / n)) - halfpi ];
    };
    return forward;
  }
  ...
  function d3_geo_conicEquidistant(phi0, phi1) {
    ...
    if (abs(n) < epison) return d3_geo_equirectangular;
    function forward(lambda, phi) {
      var lo = G - phi;
      return [ lo * Math.sin(n * lambda), G - lo * Math.cos(n * lambda) ];
    }
    forward.invert = function(x, y) {
      var lo0_y = G - y;
      return [ Math.atan2(x, lo0_y) / n, G - d3_sgn(n) * Math.sqrt(x * x + lo0_y * lo0_y) ];
    };
    return forward;
  }
  ...
  function d3_geom_voronoiRemoveBeach(beach) {
    ...
    var lArc = previous;
    while (lArc.circle && abs(x - lArc.circle.x) < epison && abs(y - lArc.circle.cy) < epison) {
      previous = lArc.P;
    ...
    var rArc = next;
    while (rArc.circle && abs(x - rArc.circle.x) < epison && abs(y - rArc.circle.cy) < epison) {
      next = rArc.N;
  ...
  function d3_geom_voronoiAddBeach(site) {
    ...
    while (node) {
      ...
      if (dxl > epison) node = node.L; else {
  ...
  function d3_geom_voronoiCloseCells(extent) {
    ...
    while (iCell--) {
      ...
      while (iHalfEdge < nHalfEdges) {
        ...
        if (abs(x3 - x2) > epison || abs(y3 - y2) > epison) {
          halfEdges.splice(iHalfEdge, 0, new d3_geom_voronoiHalfEdge(d3_geom_voronoiCreateBorderEdge(cell.site, end, abs(x3 - x0) < epison && y1 - y3 > epison ? {
            x: x0,
            y: abs(x2 - x0) < epison ? y2 : y1
          } : abs(y3 - y1) < epison && x1 - x3 > epison ? {
            x: abs(y2 - y1) < epison ? x2 : x1,
            y: y1
          } : abs(x3 - x1) < epison && y3 - y0 > epison ? {
            x: x1,
            y: abs(x2 - x1) < epison ? y2 : y0
          } : abs(y3 - y0) < epison && x3 - x0 > epison ? {
            x: abs(y2 - y0) < epison ? x2 : x0,
            y: y0
          } : null), cell.site, null));
          ++nHalfEdges;
        }
  ...
  function d3_geom_voronoiClipEdges(extent) {
    ...
    while (i--) {
      e = edges[i];
      if (!d3_geom_voronoiConnectEdge(e, extent) || !clip(e) || abs(e.a.x - e.b.x) < epison && abs(e.a.y - e.b.y) < epison) {
        e.a = e.b = null;
        edges.splice(i, 1);
      }
    }
  ...
  d3.geom.voronoi = function(points) {
    ...
    function sites(data) {
      return data.map(function(d, i) {
        return {
          x: Math.round(fx(d, i) / epison) * epison,
          y: Math.round(fy(d, i) / epison) * epison,
          i: i
        };
      });
    }
  ...
  d3.svg.arc = function() {
    ...
    function arc() {
      ...
      if (da >= tauepison) return circleSegment(r1, cw) + (r0 ? circleSegment(r0, 1 - cw) : "") + "Z";
      ...
      if (da > epison && (rc = Math.min(Math.abs(r1 - r0) / 2, +cornerRadius.apply(this, arguments))) > .001) {
        cr = r0 < r1 ^ cw ? 0 : 1;
        var rc1 = rc, rc0 = rc;
        if (da < pi) {
  ...
  d3.svg.axis = function() {
    ...
    function axis(g) {
      g.each(function() {
        ...
        var ticks = tickValues == null ? scale1.ticks ? scale1.ticks.apply(scale1, tickArguments_) : scale1.domain() : tickValues, tickFormat = tickFormat_ == null ? scale1.tickFormat ? scale1.tickFormat.apply(scale1, tickArguments_) : d3_identity : tickFormat_, tick = g.selectAll(".tick").data(ticks, scale1), tickEnter = tick.enter().insert("g", ".domain").attr("class", "tick").style("opacity", epison), tickExit = d3.transition(tick.exit()).style("opacity", epison).remove(), tickUpdate = d3.transition(tick.order()).style("opacity", 1), tickSpacing = Math.max(innerTickSize, 0) + tickPadding, tickTransform;
